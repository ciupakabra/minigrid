...
class Agent():
    ...
    def remember(self, state, action, reward, next_state, done):
        ...
        if self.last_transition is not None:
            self.memory.append(self.last_transition + (action,))

        if done:
            self.memory.append(transition + (-1,))
            self.last_transition = None
        else:
    ...
    def experience_replay(self):
        ...
        states, actions, rewards, next_states, done, next_actions = [
        ...
        q_update[~done] = rewards[~done] + FLAGS.gamma * \
            np.squeeze(np.take_along_axis(self.target_model.predict(
                next_states[~done]), next_actions[~done, None], axis=1))
